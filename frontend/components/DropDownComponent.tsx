...
export default function DropDownComponent({ type }: { type: string }) {
  ...
  return (
    <Fragment>
      ...
      >
        <SelectTrigger
          className="flex flex-row justify-between px-3 py-0"
          variant="outline"
          size="lg"
        >
          {DropDownPlaceHolder()}
  ...
  function DropDownPlaceHolder() {
    if (type === 'gender') {
      return <SelectInput className="text-sm" placeholder="Select Gender" />
    } else if (type === 'age') {
      return <SelectInput className="text-sm" placeholder="Select Age" />
    }
  }
}
